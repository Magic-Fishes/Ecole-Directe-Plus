...
const Lab = lazy(() =>
  import("./components/app/CoreApp").then((module) => {
    return { default: module.Lab };
  })
);
const Museum = lazy(() =>
  import("./components/app/CoreApp").then((module) => {
    return { default: module.Museum };
  })
);
const UnsubscribeEmails = lazy(() =>
  import("./components/app/CoreApp").then((module) => {
    return { default: module.UnsubscribeEmails };
  })
);
const Header = lazy(() =>
  import("./components/app/CoreApp").then((module) => {
...
const LoginBottomSheet = lazy(() =>
  import("./components/app/CoreApp").then((module) => {
    return { default: module.LoginBottomSheet };
  })
);
  ...
  console.log(
    ...
    `color:${
      window.matchMedia("(prefers-color-scheme: dark)").matches
        ? "rgb(223, 98, 98)"
        : "rgb(200, 80, 80)"
    };font-size:1.5rem;-webkit-text-stroke: 1px black;font-weight:bold`,
    ""
  );
}
...
const referencedErrors = {
  ...
  74000:
    "La connexion avec le serveur a échoué, réessayez dans quelques minutes",
...
const browserExtensionDownloadLink = {
  Opera:
    "https://chromewebstore.google.com/detail/ecole-directe-plus-unbloc/jglboadggdgnaicfaejjgmnfhfdnflkb?hl=fr",
  Chromium:
    "https://chromewebstore.google.com/detail/ecole-directe-plus-unbloc/jglboadggdgnaicfaejjgmnfhfdnflkb?hl=fr",
  Chrome:
    "https://chromewebstore.google.com/detail/ecole-directe-plus-unbloc/jglboadggdgnaicfaejjgmnfhfdnflkb?hl=fr",
...
const accountListFromLs = JSON.parse(
  localStorage.getItem("accountsList") ?? "[]"
);
const oldActiveAccountFromLs = parseInt(
  localStorage.getItem("oldActiveAccount") ?? 0
);
let userSettingsFromLs = JSON.parse(
...
function getSetting(setting, accountIdx, isGlobal = false) {
  if (isGlobal) {
    const globalSettingsFromLs = JSON.parse(
    ...
    userSettingsFromLs = JSON.parse(
      localStorage.getItem("userSettings") ?? "{}"
    );
    return (
      (userSettingsFromLs[accountIdx] &&
        userSettingsFromLs[accountIdx][setting]) ??
...
function initSettings(accountList) {
  ...
  for (let i = 0; i < (accountList?.length || 1); i++) {
    //Si au login, il y a aucun compte d'enregistré on considère qu'il y a un seul compte
    userSettings.push({
      displayTheme: {
...
window.addEventListener("beforeinstallprompt", (event) => {
  ...
  promptInstallPWA = () => event.prompt();
});
window.addEventListener("appinstalled", () => {
  promptInstallPWA = null;
});
...
export default function App({ edpFetch }) {
  ...
  const [keepLoggedIn, setKeepLoggedIn] = useState(
    getSetting("keepLoggedIn", activeAccount, true)
  ); // fonctionnalité "rester connecté"

  // user settings
  const [userSettings, setUserSettings] = useState(
    initSettings(accountListFromLs)
  ); // paramètres propre à chaque profil du compte
  const [shareSettings, setShareSettings] = useState(
    getSetting("shareSettings", activeAccount, true)
  );
  const [isDevChannel, setIsDevChannel] = useState(
    getSetting("isDevChannel", activeAccount, true)
  ); // canal dev: redirige vers l'URL dev.ecole-directe.plus où on déploie beaucoup plus régulièrement les mises à jour, mais qui peut être un peu instable
  ...
  const [isMobileLayout, setIsMobileLayout] = useState(
    () =>
      window.matchMedia(
        `(max-width: ${WINDOW_WIDTH_BREAKPOINT_MOBILE_LAYOUT}px)`
      ).matches
  ); // permet de modifier le layout en fonction du type d'écran pour améliorer le responsive
  const [isTabletLayout, setIsTabletLayout] = useState(
    () =>
      window.matchMedia(
        `(max-width: ${WINDOW_WIDTH_BREAKPOINT_TABLET_LAYOUT}px)`
      ).matches
  );
  ...
  ); // détermine si l'utilisateur a installé le site comme application, permet également de modifier le layout en conséquence
  ...
  class File {
    ...
    fetch() {
      if (!this.blob) {
        ...
        fetchFile(this.id, this.type, this.specialParams).then((blob) => {
          this.blob = blob;
          if (this.state === "requestForInstall") {
            this.install();
          }
  ...
  function syncSettings() {
    setUserSettings((oldSettings) => {
      ...
      const newSettings = Array.from({ length: oldSettings.length }, (_) =>
        structuredClone(selectedUserSetting)
      );
      return newSettings;
    });
  }
  ...
  useEffect(() => {
    ...
    const handleStorageChange = () => {
      const newLsGlobalSettings = JSON.parse(
        localStorage.getItem("globalSettings")
      );
  ...
  }, [keepLoggedIn, shareSettings, isDevChannel]);
    ...
    if (userSettings?.length > 0) {
      ...
      for (let i = 0; i < userSettings.length; i++) {
        ...
        for (let n in userSettings[i]) {
          lsUserSettings[i][n] = userSettings[i]
            ? userSettings[i][n]?.value ?? defaultSettings[n]
            : defaultSettings[n];
        }
      }
  ...
  const useUserData = (data = "") =>
    data
      ? {
          set: (value) => changeUserData(data, value),
          get: () => getUserData(data),
        } // If the data is choosen, there is no need to get the full
  ...
  function applyConfigFromLocalStorage() {
    ...
    if (
      ...
      !areOccurenciesEqual(accountsList, accountsListState)
    ) {
        ...
        if (window.innerWidth >= 869 && window.innerWidth < 1250) {
          if (window.innerWidth >= 995) {
            document.documentElement.style.zoom =
              (0.2 / 170) * window.innerWidth - 0.47;
          } else {
          ...
          if (isSafari) {
            const newFontSize = (0.125 / 170) * window.innerWidth - 0.294;
            if (newFontSize < 8) {
  ...
  function deleteFakeGrade(UUID, subjectKey, periodKey) {
    ...
    newGrades[periodKey].subjects[subjectKey].grades = newGrades[
      periodKey
    ].subjects[subjectKey].grades.filter((el) => el.id !== UUID);
  ...
  function updatePeriodGrades(periodKey) {
    ...
    for (const subject in period.subjects) {
      if (!subject.includes("category")) {
        period.subjects[subject].average = calcAverage(
          period.subjects[subject].grades
        );
      } else {
        period.subjects[subject].average = calcCategoryAverage(
          period,
  ...
  function sortGrades(grades, activeAccount) {
    ...
    if (periodsFromJson !== undefined) {
      for (let period of periodsFromJson) {
        if (period) {
          ...
          for (let matiere of period.ensembleMatieres.disciplines) {
            ...
            if (matiere.sousMatiere) {
              newSubject.name =
                matiere.codeMatiere + " - " + matiere.codeSousMatiere;
            } else {
              newSubject.name = matiere.discipline
                .replace(". ", ".")
                .replace(".", ". ");
            }
          ...
          generalAverageHistory[period.codePeriode] = {
            generalAverages: [],
            dates: [],
          };
          classGeneralAverageHistory[period.codePeriode] = {
      ...
      const lastGrades = [...gradesFromJson]
        .sort(
          (elA, elB) =>
            new Date(elA.dateSaisie).getTime() -
            new Date(elB.dateSaisie).getTime()
        )
        .slice(-3);
      ...
      for (let grade of gradesFromJson ?? []) {
        ...
        if (periods[tempPeriodCode].isMockExam) {
          ...
          if (periods[newPeriodCode] === undefined) {
            newPeriodCode =
              Object.keys(periods)[
                Object.keys(periods).indexOf(tempPeriodCode) - 1
              ];
            newPeriodCode =
              Object.keys(periods)[
                Object.keys(periods).indexOf(tempPeriodCode) - 1
        ...
        newGrade.examSubjectSRC =
          grade.uncSujet === ""
            ...
            : new File(
                ...
                "NODEVOIR",
                grade.uncSujet,
                `sujet-${grade.devoir}-${grade.subjectCode}`,
                { idDevoir: grade.id }
              );
        newGrade.examCorrectionSRC =
          grade.uncCorrige === ""
            ? undefined
            : new File(
                grade.uncCorrige,
                "NODEVOIR",
                grade.uncCorrige,
                `correction-${grade.devoir}-${grade.subjectCode}`,
                { idDevoir: grade.id }
              );
        ...
        subjectDatas[periodCode][subjectCode].push({
          value: newGrade.value,
          coef: newGrade.coef,
          scale: newGrade.scale,
          isSignificant: newGrade.isSignificant,
          classAverage: newGrade.classAverage,
        });
        const nbSubjectGrades =
          periods[periodCode].subjects[subjectCode]?.grades.filter(
            (el) => el.isSignificant
          ).length ?? 0;
        const subjectAverage =
          periods[periodCode].subjects[subjectCode].average;
        const oldGeneralAverage = isNaN(periods[periodCode].generalAverage)
          ? 10
          : periods[periodCode].generalAverage;
        const average = calcAverage(subjectDatas[periodCode][subjectCode]);
        const classAverage = calcClassAverage(
          subjectDatas[periodCode][subjectCode]
        );

        // streak management
        newGrade.upTheStreak =
          !isNaN(newGrade.value) &&
          newGrade.isSignificant &&
        ...
        if (newGrade.upTheStreak) {
          ...
          if (
            newGrade.isSignificant &&
            !["Abs", "Disp", "NE", "EA", "Comp"].includes(newGrade.value)
          ) {
            periods[periodCode].streak -=
              periods[periodCode].subjects[subjectCode].streak;
            periods[periodCode].subjects[subjectCode].streak = 0;

            // enlève le "upTheStreak" des notes précédant celle qu'on considère
            for (let grade of periods[periodCode].subjects[subjectCode]
              .grades) {
        ...
        if (category !== null) {
          const categoryAverage = calcCategoryAverage(
            periods[periodCode],
            category
          );
        ...
        const classGeneralAverage = calcClassGeneralAverage(
          periods[periodCode]
        );
        classGeneralAverageHistory[periodCode].classGeneralAverages.push(
          classGeneralAverage
        );
        ...
        if (!isNaN(newGrade.value)) {
          if (newGrade.value === newGrade.scale) {
            // si la note est au max on donne l'étoile (le parfait)
            gradeBadges.push("star");
            periods[periodCode].subjects[subjectCode].badges.star++;
            totalBadges.star++;
          }
          if (newGrade.value === newGrade.classMax) {
            // si la note est la meilleure de la classe on donne le plus
          ...
          if (newGrade.value > newGrade.classAverage) {
            // si la note est > que la moyenne de la classe on donne le badge checkBox tier
          ...
          if ((newGrade.value / newGrade.scale) * 20 > subjectAverage) {
            // si la note est > que la moyenne de la matiere on donne le badge stonks tier
            gradeBadges.push("stonks");
            periods[periodCode].subjects[subjectCode].badges.stonks++;
            totalBadges.stonks++;
          }
          if (newGrade.upTheStreak) {
            // si la note up la streak on donne le badge de streak
            gradeBadges.push("keepOnFire");
            periods[periodCode].subjects[subjectCode].badges.keepOnFire++;
            totalBadges.keepOnFire++;
          }
          if ((newGrade.value / newGrade.scale) * 20 === subjectAverage) {
            // si la note est = à la moyenne de la matiere on donne le badge = tier
    ...
    for (const period in periods) {
      for (const subject in periods[period].subjects) {
        for (const subjectID in subjectsComparativeInformation[period]) {
          if (
            periods[period].subjects[subject].name ===
            subjectsComparativeInformation[period][subjectID].subjectFullname
          ) {
            const newAverage = periods[period].subjects[subject].average;
            if (
              newAverage === "N/A" ||
              periods[period].subjects[subject].classAverage === "N/A" ||
              periods[period].subjects[subject].code.includes("category")
            ) {
              subjectsComparativeInformation[period].splice(subjectID, 1);
              break;
            }
            subjectsComparativeInformation[period][subjectID].average =
              newAverage;
            break;
          }
        }
  ...
  function sortNextHomeworks(homeworks) {
    // This function will sort (I would rather call it translate) the EcoleDirecte response to a better js object
    const upcomingAssignments = [];
    const sortedHomeworks = Object.fromEntries(
      Object.entries(homeworks).map((day) => {
        return [
          day[0],
          day[1].map((homework, i) => {
            const {
              codeMatiere,
              aFaire,
              donneLe,
  ...
  function sortDayHomeworks(homeworks) {
    // This function will sort (I would rather call it translate) the EcoleDirecte response to a better js object
    const sortedHomeworks = Object.fromEntries(
      Object.entries(homeworks).map((day) => {
        return [
            ...
            .map((homework) => {
              ...
              if (aFaire) {
                ...
                return {
                  ...
                  content: contenu,
                  files: documents.map(
                    (e) => new File(e.id, e.type, e.libelle)
                  ),
                  sessionContent: contenuDeSeance.contenu,
                  sessionContentFiles: contenuDeSeance.documents.map(
                    (e) => new File(e.id, e.type, e.libelle)
                  ),
                };
              } else {
  ...
  function sortMessages(messages) {
    const sortedMessages = messages.messages.received.map((message) => {
      return {
        date: message.date,
        files: structuredClone(message.files)?.map(
          (file) => new File(file.id, file.type, file.libelle)
        ),
        from: message.from,
        id: message.id,
        read: message.read,
  ...
  function sortMessageContent(messageContent) {
    ...
    const targetMessageIdx = oldSortedMessages.findIndex(
      (item) => item.id === messageContent.id
    );
    oldSortedMessages[targetMessageIdx].read = true;
    oldSortedMessages[targetMessageIdx].files = messageContent.files.map(
      (file) => new File(file.id, file.type, file.libelle)
    );
  ...
  function sortSchoolLife(schoolLife, activeAccount) {
    ...
    schoolLife[activeAccount]?.absencesRetards
      .concat(schoolLife[activeAccount].sanctionsEncouragements ?? [])
      .forEach((item) => {
        const newItem = {};
  ...
  function handleEdBan() {
    ...
    createNotification(
      <>
        <h4>Installez Ecole Directe Plus Unblock</h4>
        <hr />
        <div className="edpu-notification-description">
          <EdpuLogo />
          <p>
            Ecole Directe Plus a besoin de son extension de navigateur pour
            fonctionner. (fourni un accès continu à l'API d'EcoleDirecte)
          </p>
        </div>
        <hr />
        <div className="extension-download-link">
          ...
          <a
            href={browserExtensionDownloadLink[userBrowser]}
            target={
              !["Safari", "Firefox"].includes(userBrowser) ? "_blank" : ""
            }>
            Télécharger
          </a>
        </div>
  ...
  async function fetchLogin(
    ...
    edpFetch(
      getProxiedURL(
        `https://api.ecoledirecte.com/v3/login.awp?v=${apiVersion}`,
        true
      ),
      options,
      "text"
        ...
        if (statusCode === 200) {
          ...
          if (accountType === "E") {
            ...
            accountsList.push({
              ...
              class: accounts.profile.classe
                ? [
                    accounts.profile.classe.code,
                    accounts.profile.classe.libelle,
                  ]
                : ["inconnu", "inconnu"], // classe de l'élève, code : 1G4, libelle : Première G4
            ...
            accounts.profile.eleves.map((account) => {
              accountsList.push({
                ...
                class: account.classe
                  ? [account.classe.code, account.classe.libelle]
                  : ["inconnu", "inconnu"], // classe de l'élève, code : 1G4, libelle : Première G4
          ...
          if (
            accountsListState.length > 0 &&
            (accountsListState.length !== accountsList.length ||
              accountsListState[0].id !== accountsList[0].id)
          ) {
            resetUserData();
          }
          setUserInfo(token, accountsList);
      ...
      .finally(() => {
        loginAbortControllers.current.forEach((e) => {
          e.abort();
        });
        callback(messages);
      });
  ...
  async function fetchUserTimeline(controller = new AbortController()) {
    ...
    const data = {
      anneeScolaire: getUserSettingValue("isSchoolYearEnabled")
    ...
    edpFetch(
      getProxiedURL(
        `https://api.ecoledirecte.com/v3/eleves/${accountsListState[activeAccount].id}/timeline.awp?verbe=get&v=${apiVersion}`,
        true
      ),
      {
        method: "POST",
  ...
  async function fetchHomeworksSequentially(
    controller = new AbortController(),
    date = "incoming"
  ) {
    abortControllers.current.push(controller);
        ...
        if (code === 200) {
          if (date === "incoming") {
            ...
            changeUserData("sortedHomeworks", {
              ...getUserData("sortedHomeworks"),
              ...sortDayHomeworks({
                [responseData.data.date]: responseData.data.matieres,
              }),
            });
          }
        } else if (code === 520 || code === 525) {
  ...
  async function fetchHomeworksDone(
    ...
    return edpFetch(
      ...
      ),
      {
        ...
        body:
          "data=" +
          JSON.stringify({
            idDevoirsEffectues: tasksDone,
            idDevoirsNonEffectues: tasksNotDone,
          }),
        signal: controller.signal,
      },
      "json"
  ...
  async function fetchMessages(controller = new AbortController()) {
    ...
    const userId = activeAccount;
    const data = {
      anneeMessages: getUserSettingValue("isSchoolYearEnabled")
        ? getUserSettingValue("schoolYear").join("-")
    ...
    edpFetch(
      getProxiedURL(
        `https://api.ecoledirecte.com/v3/${
          accountsListState[userId].accountType === "E"
            ? "eleves/" + accountsListState[userId].id
            : "familles/" + accountsListState[userId].familyId
        }/messages.awp?force=false&typeRecuperation=received&idClasseur=0&orderBy=date&order=desc&query=&onlyRead=&page=0&itemsPerPage=100&getAll=0&verbe=get&v=${apiVersion}`,
        true
      ),
      {
        method: "POST",
      ...
      getProxiedURL(
        `https://api.ecoledirecte.com/v3/${
          accountsListState[userId].accountType === "E"
            ? "eleves/" + accountsListState[userId].id
            : "familles/" + accountsListState[userId].familyId
        }/messages/${id}.awp?verbe=get&mode=destinataire&v=${apiVersion}`,
        true
      ),
      {
        method: "POST",
      ...
      getProxiedURL(
        `https://api.ecoledirecte.com/v3/${
          accountsListState[userId].accountType === "E"
            ? "eleves/" + accountsListState[userId].id
            : "familles/" + accountsListState[userId].familyId
        }/messages.awp?verbe=put&v=${apiVersion}`,
        true
      ),
      {
        method: "POST",
  ...
  async function fetchSchoolLife(controller = new AbortController()) {
    ...
    edpFetch(
      getProxiedURL(
        `https://api.ecoledirecte.com/v3/eleves/${accountsListState[activeAccount].id}/viescolaire.awp?verbe=get&v=${apiVersion}`,
        true
      ),
      {
        method: "POST",
      ...
      .then((response) => {
        ...
        if (code === 200 || code === 210) {
          // 210: quand l'utilisateur n'a pas de retard/absence/sanction
  ...
  async function createFolderStorage(name) {
    ...
    edpFetch(
      getProxiedURL(
        "https://api.ecoledirecte.com/v3/messagerie/classeurs.awp?verbe=post%26v=4.52.0",
        true
      ),
      {
        method: "POST",
  ...
  useEffect(() => {
    ...
    if (getUserSettingValue("displayTheme") === "dark") {
      ...
      metaThemeColor.content = window.matchMedia("(prefers-color-scheme: dark)")
        .matches
        ? "#181829"
        : "#e4e4ff";
    }
  ...
  function toggleThemeTransitionAnimation() {
    if (
      getUserSettingValue("displayMode") === "balanced" ||
      getUserSettingValue("displayMode") === "performance"
    ) {
      return 0;
    }
  ...
  const router = createBrowserRouter([
    {
      ...
      element: (
        <Root
          ...
          setDisplayTheme={(value) => {
            changeUserSettings("displayTheme", value);
          }}
          displayTheme={getUserSettingValue("displayTheme")}
          setDisplayModeState={(value) => {
            changeUserSettings("displayMode", value);
          }}
      ...
      children: [
        {
          element: (
            <LandingPage token={tokenState} accountsList={accountsListState} />
          ),
          path: "/",
        },
        {
          element: (
            <Feedback
              activeUser={
                accountsListState.length > 0 && accountsListState[activeAccount]
              }
        ...
        },
        {
          element: (
            <Login
              ...
              A2FInfo={A2FInfo}
              setRequireA2F={setRequireA2F}
              bufferUserIds={bufferUserIds}
              fetchLogin={fetchLogin}
              logout={logout}
              loginFromOldAuthInfo={loginFromOldAuthInfo}
              currentEDPVersion={currentEDPVersion}
            />
          ),
          path: "login",
        ...
        {
          element:
            !tokenState || accountsListState.length < 1 ? (
          ...
          children: [
            {
              element: (
                <Navigate to={`/app/${activeAccount}/account`} replace={true} />
              ),
              path: "account",
            },
            {
              element: (
                <Account
                  ...
                  fetchSchoolLife={fetchSchoolLife}
                  sortSchoolLife={sortSchoolLife}
                  isLoggedIn={isLoggedIn}
                  activeAccount={activeAccount}
                />
              ),
            ...
            },
            {
              element: (
                <Settings
                  usersSettings={userSettings[activeAccount]}
                  accountsList={accountsListState}
            ...
            },
            {
              element: (
                <Navigate
                  to={`/app/${activeAccount}/dashboard`}
                  replace={true}
                />
              ),
              path: ":userId",
            ...
            },
            {
              element: (
                <Dashboard
                  fetchUserGrades={fetchUserGrades}
                  grades={grades}
                  fetchHomeworks={fetchHomeworks}
                  activeAccount={activeAccount}
                  isLoggedIn={isLoggedIn}
            ...
            {
              element: (
                <Navigate to={`/app/${activeAccount}/grades`} replace={true} />
              ),
              path: "grades",
            },
            {
            ...
            },
            {
              element: (
                <Navigate
                  to={`/app/${activeAccount}/homeworks`}
                  replace={true}
                />
              ),
              path: "homeworks",
            ...
            },
            {
              element: (
                <Navigate
                  to={`/app/${activeAccount}/timetable`}
                  replace={true}
                />
              ),
              path: "timetable",
            ...
            },
            {
              element: (
                <Navigate
                  to={`/app/${activeAccount}/messaging`}
                  replace={true}
                />
              ),
              path: "messaging",
            },
            {
              element: (
                <Messaging
                  ...
                  fetchMessageMarkAsUnread={fetchMessageMarkAsUnread}
                />
              ),
