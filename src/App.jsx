...
function sortGrades(grades, activeAccount) {
    ...
    for (const key in periods) {
        ...
        if (i === Object.keys(periods).length - 1) {
            lastPeriod = { key: key, value: periods[key] }
        }
        i++;
        let isEmpty = true;
        if (periods[key])
    ...
    // Ajoute la période annuelle
    if (firstPeriod !== undefined && lastPeriod !== undefined) {
        const annualPeriod = {
            code: "allYear",
            name: "Année complète",
            startDate: firstPeriod.value.startDate,
        ...
        // Merge all subjects and grades from all periods
        for (const periodCode in periods) {
            const period = periods[periodCode];
            for (const subjectCode in period.subjects) {
                if (!annualPeriod.subjects[subjectCode]) {
                    annualPeriod.subjects[subjectCode] = {
                        code: subjectCode,
                        elementType: "subject",
                        name: period.subjects[subjectCode].name,
                        classAverage: "N/A",
                        minAverage: "N/A",
                        maxAverage: "N/A",
                        coef: period.subjects[subjectCode].coef,
                        size: period.subjects[subjectCode].size,
                        rank: period.subjects[subjectCode].rank,
                        isCategory: period.subjects[subjectCode].isCategory,
                        teachers: period.subjects[subjectCode].teachers,
                        appreciations: period.subjects[subjectCode].appreciations,
                        grades: [],
                        average: "N/A",
                        streak: 0,
                        badges: {
                ...
                annualPeriod.subjects[subjectCode].grades.push(...period.subjects[subjectCode].grades);
            }
        }

        // Calculate averages and other statistics for the annual period
        for (const subjectCode in annualPeriod.subjects) {
            const subject = annualPeriod.subjects[subjectCode];
            subject.average = calcAverage(subject.grades.map(g => ({ value: g.value, coef: g.coef, scale: g.scale, isSignificant: g.isSignificant })));
            const generalAverage = calcGeneralAverage(annualPeriod);
            annualPeriod.generalAverage = generalAverage;
            ...
            subject.maxStreak = maxStreak;
        }

        periods["allYear"] = annualPeriod;
    }
